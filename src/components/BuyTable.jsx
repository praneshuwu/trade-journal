...
function BuyTable({ trade, dispatch }) {
  const isClosed = trade.status === "CLOSED"
  const [modal, setModal] = useState(null)

  function handleDelete(e, legId) {
    e.stopPropagation()
    dispatch({ type: "DELETE_LEG", tradeId: trade.trade_id, legId, side: "buy" })
  }
    ...
    <div className="p-4 pl-6">
        ...
        <tbody>
          {trade.buy.map((leg) => (
            <tr
              ...
              onMouseEnter={(e) => { if (!isClosed) e.currentTarget.style.backgroundColor = "var(--long-glow)" }}
      ...
      {!isClosed && (
        <button
          ...
          style={{ color: "var(--long-text)", borderColor: "rgba(21,128,61,0.4)", background: "var(--long-badge)" }}
          onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "rgba(21,128,61,0.3)")}
          onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "transparent")}
        >
          + Buy
        </button>
